	...
	protected void analyzeBuffer(byte[] data, int off, int len) {
		if (data[off + 0] == 100 && data[off + 1] == 88 && data[off + 2] == 32 && data[off + 3] == 37) {
			...
			if (framesize == 0 || dtsHD) {
				...
				int framesize_sup = 0;
				int dts_rate = 48000;
				if (!rendererconfiguration.isSkipDTSHDst() && off + framesize + 3 < data.length && data[off + framesize] == 100 && data[off + framesize + 1] == 88 && data[off + framesize + 2] == 32 && data[off + framesize + 3] == 37) {
					dtsHD = true;
					dts_rate = 192000;
